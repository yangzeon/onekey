...
judge(){
	if [[ $? -eq 0 ]];then
		...
		echo -e "${Info} ${GreenBG} 脚本终止继续安装 反馈地址： ${Font}"
		exit 1
	fi
}
...
echo -e "${OK} ${GreenBG} 访问网站首页查看 http://${getdomain} ${Font}"
	...
	echo -e "${OK} ${GreenBG} wget -N --no-check-certificate https://raw.githubusercontent.com/yangzeon/onekey/master/c.sh && chmod +x c.sh && bash c.sh ${Font}"
	exit 1
fi
}
